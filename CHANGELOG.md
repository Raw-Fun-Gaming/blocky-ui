...
### Fixed
- Close button default `top`/`right` offset tightened from `--rfui-padding-md` (12px) to `--rfui-padding-sm` (8px)

## [2.1.2] - 2026-04-23

### Added
- `RawFunUI.showFatalError(title, message)` — blocking error modal with no close button and no overlay-click dismiss; use for fatal/game-breaking errors where the session cannot continue

### Changed
- `RawFunUI.showError` now shows a close button and allows overlay-click dismiss — use for soft/recoverable errors
- `.rfui-modal-body` text is now center-aligned by default
